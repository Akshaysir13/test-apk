// src/pages/Dashboard.tsx - IMPROVED UX WITH SHOW/HIDE LOCKED TESTS
...
export default function Dashboard() {
  ...
  const [showLockedTests, setShowLockedTests] = useState(false); // NEW: Toggle for locked tests
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        {/* Stats - IMPROVED: Only show relevant stats */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <p className="text-sm text-gray-500 mt-1">Ready to practice</p>
          </div>
        ...
        {/* Accessible Tests - MAIN SECTION */}
        {filteredAccessibleTests.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              ✅ Your Tests ({filteredAccessibleTests.length})
            </h2>
        ...
        {/* Upgrade Section - IMPROVED: Compact CTA */}
        {lockedTests.length > 0 && (
          <div className="mt-8">
            {/* Show/Hide Locked Tests Toggle */}
            <div className="mb-4 flex items-center justify-between bg-white p-4 rounded-lg shadow">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  🔒 {lockedTests.length} More Tests Available
                </h3>
                <p className="text-sm text-gray-600">
                  Upgrade to unlock premium test series
                </p>
              </div>
              <button
                onClick={() => setShowLockedTests(!showLockedTests)}
                className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold"
              >
                {showLockedTests ? 'Hide Locked Tests' : 'View Locked Tests'}
              </button>
            </div>

            {/* Locked Tests Grid - Only show when toggled */}
            {showLockedTests && filteredLockedTests.length > 0 && (
              <div className="mb-8">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  🔒 Locked Tests ({filteredLockedTests.length})
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {/* Upgrade Cards - IMPROVED: Always visible but compact */}
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 relative">
            ...
            <div className="text-center">
              ...
              <div className="bg-gray-100 p-4 rounded-lg mb-4">
                ...
                <p className="text-sm text-gray-600">Phone: +91 7028140857</p>
              </div>
