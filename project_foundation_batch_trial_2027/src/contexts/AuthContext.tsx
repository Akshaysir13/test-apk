...
function shouldApplyLoginRateLimit(courses: string[] | undefined): boolean {
  if (!courses?.length) return false;

  const paidCourses = ['foundation', 'rank_booster', 'advance_2026', 'nata_2026'];
  const normalized = courses.map((c) => c.toLowerCase().replace(/[-_\s]/g, '_'));
  const hasPaidCourse = normalized.some((c) => paidCourses.includes(c));

  if (hasPaidCourse) return false;
  return isRateLimitedCourse(courses);
}

function enforceLoginRateLimit(email: string, courses: string[] | undefined): LoginResult | null {
  if (!shouldApplyLoginRateLimit(courses)) return null;

  if (isLoginRateLimited(email)) {
    return { success: false, message: LOGIN_RATE_LIMIT_MESSAGE };
  }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<LoginResult> => {
    ...
    const rateLimitCourses = matchedByEmail?.courses ?? ['free_tests'];
    const rateLimitBlock = enforceLoginRateLimit(normalizedEmail, rateLimitCourses);
